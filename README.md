...
## Documentos manuales del Quedan

Para pagos creados manualmente sin facturas en Odoo, la sección **Documentos del Quedan** del pago permite capturar líneas (Documento, Referencia, Fecha, Monto) que se imprimen en el Quedan.

- Son **exclusivamente informativas**: el modelo `account.supplier.quedan.document` no tiene vínculo con `account.move` y no modifica `invoice_ids`, `reconciled_bill_ids`, asientos, vencimientos ni el importe del pago.
- **Prioridad**: si el pago tiene líneas manuales, el Quedan usa **solo** esas; si no, usa las facturas relacionadas contablemente (`reconciled_bill_ids`); si no hay ninguna, la lista queda vacía. Las dos fuentes nunca se mezclan.
- Si la suma de las líneas manuales no coincide con el importe del pago (comparado con la precisión de la moneda), se muestra una advertencia. No bloquea guardar ni imprimir. Con facturas automáticas no se compara, porque el pago puede ser parcial.
- Duplicar un pago no copia sus líneas manuales.
- Permisos: igual que `account.payment` (solo lectura para *Contabilidad - Solo lectura*, lectura/escritura para *Facturación* y superiores) y regla multiempresa por la empresa del pago.
...
Variables: `company.*` (name, nit, nrc, address, street, phone, email, logo), `supplier.*` (name, nit, nrc, address, phone, email), `quedan.*` (number, date, amount, currency, payment_days, payment_date, memo, reference, payment_number, documents_count, documents_source, documents_total, documents_mismatch, is_canceled), `copy.label`, `copy.is_original` y `documents` (number, reference, date, amount, currency).
...
- **Documentos automáticos**: si el pago no tiene documentos manuales, salen de `reconciled_bill_ids` estándar de Odoo, es decir, de las facturas desde las que se registró el pago (`invoice_ids`) y de las facturas conciliadas con el asiento del pago. En un pago manual en borrador sin facturas vinculadas, la lista queda vacía hasta que se concilie, salvo que se capturen documentos manuales. El monto de cada factura es su `amount_total`, aunque el pago sea parcial.
